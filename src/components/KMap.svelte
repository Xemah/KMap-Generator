<script>
	import { form, terms, variablesCount, hoveredTerm } from '@/lib/stores';
			...
			<div class="flex flex-col border border-gray-700">
				{#each kMap.arr as row}
					<div class="flex border-b last:border-b-0 border-gray-700">
						{#each row as { index, key, term, value }}
							<div
								class={
							...
							>
								{#each groups.filter((group) => group.elements.some((el) => el.index === index)) as group}
									<div
